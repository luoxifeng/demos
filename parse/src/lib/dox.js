...
export function parseComments(js, options){
  options = options || {};
  js = js.replace(/\r\n/gm, '\n');
    ...
    if (!withinMultiline && !withinSingle && !withinString &&
      ...
      if (buf.trim().length) {
        ...
        if(comment) {
          ...
          comment.code = code = trimIndentation(buf).trim();
          comment.ctx = parseCodeContext(code, parentContext);
...
export function trimIndentation (str) {
...
export function parseComment(str, options) {
  str = str.trim();
  options = options || {};
  ...
  // parse tags
  if (tags.length) {
    comment.tags = tags.slice(1).map(parseTag);
...
export function extractTagParts(str) {
  var level = 0,
    extract = '',
    split = [];
...
export function parseParamOptional(tag) {
...
export const parseCodeContext = function(str, parentContext) {
  parentContext = parentContext || {};

  var ctx;

  // loop through all context matchers, returning the first successful match
  return contextPatternMatchers.some(function (matcher) {
    return ctx = matcher(str, parentContext);
  }) && ctx;
};

export const contextPatternMatchers = [
